...
This document explains how you use the code generated by the SwiftProtobufAlias plugin.

The generated code relies very heavily on the associated SwiftProtobufAlias library.
...
These structs conform to `SwiftProtobufAlias.Message` and provide Swift properties for every
...
```swift
public struct Example: SwiftProtobufAlias.Message {
  ...
  public enum E: SwiftProtobufAlias.Enum { ... }
...
The resulting Swift enums conform to the `SwiftProtobufAlias.Enum` protocol which extends
...
```swift
public enum MyEnum: SwiftProtobufAlias.Enum {
...
the same underlying numeric value, SwiftProtobufAlias simply drops
...
Groups in the proto file generate Swift structs that conform to `SwiftProtobufAlias.Message`.

Messages in the proto file generate Swift structs that conform to
`SwiftProtobufAlias.Message`.

Note: There is also a `SwiftProtobufAlias._MessageImplementationBase`
protocol.  You should not refer to that directly; use
`SwiftProtobufAlias.Message` when you need to work with arbitrary groups or
messages.
...
```swift
public struct ExampleOneOf: SwiftProtobufAlias.Message {
...
implemented by SwiftProtobufAlias.
...
SwiftProtobufAlias can decode JSON in this case and can re-encode
back to JSON.  It only needs the types when translating
between dissimilar encodings.

Caveat:  SwiftProtobufAlias's Text format decoding will currently
...
For these reasons, SwiftProtobufAlias does not current support anything like the
